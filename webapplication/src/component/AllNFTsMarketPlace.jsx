...
import { buyNsale, removeFromMarket } from './SmartContracts';
              ...
              {element.files.map((file, index) =>
                <div className="p-0" style={{ 'width': '20rem', 'color': 'white' }}>
                  ...
                  <div class="card-body">
                    ...
                    {element.Owner == window.ethereum.selectedAddress ? (<button style={{ 'height': '4rem', 'width': '8rem', 'fontSize': '10px' }} id='list' onClick={() => {
                      removeFromMarket(element.tokenId, element.itemId);
                    }} class="btn btn-primary">REMOVE FROM NFTMARKET</button>) : (<button style={{ 'height': '4rem', 'width': '8rem', 'fontSize': '10px' }} id='list' onClick={() => {
                      buyNsale(element.tokenId, element.itemId, element.Amount);
                    }} class="btn btn-primary">BUY</button>)}                  </div>
                </div>
              )}
            </div>
          </div>
